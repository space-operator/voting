...
export const reduceAsync = async <TElem, TOut = TElem>(
  arr: TElem[],
  reducer: (acc: Awaited<TOut>, item: TElem) => Promise<TOut>,
  initialValue: TOut
): Promise<TOut> =>
  arr.reduce(
    async (acc, item) => reducer(await acc, item),
...
export const getTokenOwnerRecordAddressSync = (
  realm: PublicKey,
  mint: PublicKey,
  voter: PublicKey,
...
export function convertVoterWeightActionToType(
  action: VoterWeightAction
): UpdateVoterWeightRecordTypes {
  switch (action) {
    case VoterWeightAction.CastVote:
      return 'castVote';
    case VoterWeightAction.CommentProposal:
      return 'commentProposal';
    case VoterWeightAction.CreateGovernance:
      return 'createGovernance';
    case VoterWeightAction.CreateProposal:
      return 'createProposal';
    case VoterWeightAction.SignOffProposal:
      return 'signOffProposal';
  }
}

export const convertTypeToVoterWeightAction = (
  type: UpdateVoterWeightRecordTypes
): VoterWeightAction => {
  switch (type) {
    case 'castVote':
      return VoterWeightAction.CastVote;
    case 'commentProposal':
      return VoterWeightAction.CommentProposal;
    case 'createGovernance':
      return VoterWeightAction.CreateGovernance;
    case 'createProposal':
      return VoterWeightAction.CreateProposal;
    case 'signOffProposal':
      return VoterWeightAction.SignOffProposal;
  }
};
