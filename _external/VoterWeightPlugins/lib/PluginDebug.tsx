...
const Params = ({ plugin }: { plugin: VoterWeightPluginInfo }) => {
  const trimmedParams = Object.entries(plugin.params as any).filter(
    ([key]) => key !== 'reserved'
  );
  return (
    <div>
...
export const PluginDebug = () => {
  ...
  const {
    ...
    isReady,
  } = useRealmVoterWeightPlugins(kind);

  return (
    <div>
      <div>
        <h1>Hello! This is a voter weight plugin debug dashboard</h1>
      </div>
      ...
      <div>
        ...
        <ul>
          {isReady &&
            (plugins?.voterWeight || []).map((plugin, i) => (
              <li key={i}>
                <div>Plugin Name: {plugin.name}</div>
                <div>
                  Voter Weight (on-chain):{' '}
                  {plugin.weights?.map((w) => w?.toString()).join(',')}
                </div>
                <div>
                  Voter Weight (calculated):{' '}
                  {totalCalculatedVoterWeight?.details[
                    i
                  ].pluginWeight?.toString()}
                </div>
                <div>
                  Voter Weight (error):{' '}
                  {totalCalculatedVoterWeight?.details[i].error?.stack}
                </div>
                <div>
                  Params: <Params plugin={plugin} />
                </div>
              </li>
            ))}
        </ul>
      ...
      <div>
        ...
        <ul>
          {isReady &&
            (plugins?.maxVoterWeight || []).map((plugin, i) => (
              <li key={i}>
                <div>Plugin Name: {plugin.name}</div>
                <div>
                  Max Voter Weight (on-chain):{' '}
                  {plugin.weights?.map((w) => w?.toString()).join(',')}
                </div>
                <div>
                  Max Voter Weight (calculated):{' '}
                  {calculatedMaxVoterWeight?.details[
                    i
                  ].pluginWeight?.toString()}
                </div>
                <div>
                  Max Voter Weight (error):{' '}
                  {calculatedMaxVoterWeight?.details[i].error?.stack}
                </div>
                <div>
                  Params: <Params plugin={plugin} />
                </div>
